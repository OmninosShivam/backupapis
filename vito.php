...
class Vito extends CI_Controller
{
	...
	public function getUserFamilyDetails()
	{
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			$user = $this->db->get_where('familyMembers', ['userId' => $this->input->post('userId'), 'status' => '1'])->row_array();
			if(empty($user)){
				echo json_encode([
					'status' => 0,
	...
	public function deleteFamily()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			$leader = $this->db->get_where('family', ['userId' => $this->input->post('userId')])->row_array();
			if (empty($leader)) {
				echo json_encode([
					'status' => 0,
			...
			$this->db->delete('family', ['id' => $leader['id']]);
			echo json_encode([
				'status' => 1,
	...
	public function send_invite_to_user()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			if (!empty($check)) {
				$this->db->delete('family_invitation_to_user', ['id' => $check['id']]);
				echo json_encode([
					'status' => 2,
	...
	public function all_users()
	{
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			$lfamily = $this->db->get_where('family', ['id' => $this->input->post('familyId'), 'userId' => $user['id']])->row_array();
			if (empty($lfamily)) {
				echo json_encode([
					'status' => 0,
					'message' => 'user not a leader'
			...
			if (empty($all_users)) {
				...
				foreach ($all_users as $users) {
					...
					$invite_status = $this->db->get_where('family_invitation_to_user', ['userId' => $users['id'], 'familyId' => $lfamily['id']])->row_array();
	...
	public function send_join_request()
	{
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			// check request already sent or not if sent then remove it
			$check = $this->db->get_where('from_user_to_family_invitation', ['userId' => $user['id'], 'familyId' => $family['id']])->row_array();
			if (!!($check)) {
				$this->db->delete('from_user_to_family_invitation', ['id' => $check['id']]);
				echo json_encode([
					'status' => 2,
			...
			// check user is already in a family or not
			$check_user_in_family = $this->db->get_where('familyMembers', ['userId' => $user['id'], 'status' => '1'])->row_array();
			if (!!$check_user_in_family) {
				echo json_encode([
					'status' => 0,
			...
			// sending request to the family
			$data['userId'] = $user['id'];
			$data['familyId'] = $family['id'];
			$data['date'] = date('Y-m-d H:i:s');

			$this->db->insert('from_user_to_family_invitation', $data);
			echo json_encode([
				'status' => 1,
				'message' => 'request sent to the family'
			]);
			exit;
		} else {
			echo json_encode([
	...
	public function get_join_requests()
	{
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {

			// checking valid leader
	...
	public function accept_reject_join_requests()
	{
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			// checking valid requests
			$request = $this->db->get_where('from_user_to_family_invitation', ['id' => $this->input->post('request_id'), 'familyId' => $leader['id']])->row_array();
			if (empty($request)) {
				echo json_encode([
					'status' => 0,
			...
			if ($this->input->post('accept') == '1') {
				...
				// check user has joined another family or not
				$check = $this->db->get_where('familyMembers', ['userId' => $request['userId'], 'status' => '1'])->row_array();
				if (!!$check) {
					echo json_encode([
						'status' => 0,
						'message' => 'user already in a family'
				...
				// deleting all the requests sent by the user to other family's
				$this->db->delete('from_user_to_family_invitation', ['userId' => $request['userId']]);

				echo json_encode([
					'status' => 1,
					'message' => 'user joined the family'
				]);
				exit;
				...
				$this->db->delete('from_user_to_family_invitation', ['id' => $request['id']]);
				echo json_encode([
					'status' => 2,
	...
	public function all_family_details()
	{
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			$families = $this->db->get('family')->result_array();
			if (empty($families)) {
				echo json_encode([
					'status' => 0,
			...
			foreach ($families as $family) {
				$family['leader'] = $this->db->select('users.name, users.username, users.image')->get_where('users', ['id' => $family['userId']])->row_array();
	...
	public function get_family_details(){
		if($_SERVER['REQUEST_METHOD'] === 'POST'){
			...
			parameters_required(['familyId', 'userId']);
			...
			$family['request_status'] = false;
			$check_request = $this->db->get_where('from_user_to_family_invitation', ['userId' => $user['id'], 'familyId' => $family['id']])->row_array();
			if(!!$check_request){
				$family['request_status'] = true;
			}

			$family['family_join_status'] = false;
			$check_family_join = $this->db->get_where('familyMembers', ['familyId' => $family['id'], 'userId' => $user['id'], 'status' => '1'])->row_array();
			if(!!$check_family_join){
				$family['family_join_status'] = true;
			}
